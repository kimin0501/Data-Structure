...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        // throw exception if the child or parent node is null
        if (child == null || parent == null) {
            throw new IllegalArgumentException();
        }

        // if the child is right node of the parent, do left rotation
        if (child.isRightChild()) {

            // first step of left rotation, assign left child of the child node as new right
            // child of the parent
            parent.context[2] = child.context[1];

            // check if newly assigned child is not null, then update its parent reference
            if (parent.context[2] != null) {
                parent.context[2].context[0] = parent;
            }

            // second step of left rotation, assign the parent node's parent as the new
            // parent of the child node.
            child.context[0] = parent.context[0];

            // check if new parent of the child is not null, then update its child
            // reference,
            // whether it is right or left child of its parent
            ...
            // lastly, update new parent and child reference after rotation has been done
            child.context[1] = parent;
            parent.context[0] = child;
        }

        // if the child is left node of the parent, do right rotation
        else if (!child.isRightChild()) {

            // first step of right rotation, assign right child of the child node as new
            ...
            // second step of right rotation, assign the parent node's parent as the new
            // parent of the child node.
            child.context[0] = parent.context[0];
        ...
        // throw exception if child is neither right nor left child of the parent node
        else {
            throw new IllegalArgumentException("Provided child is not initially related");
        }
        // if the parent node was root of the tree, then assign child as new root
        if (parent == root) {
            root = child;
        }
    ...
    // This test method will check when parent node is root, and the child is right
    // child of the parent
    public static boolean test1() {
        try {

            // Form a tree with 7 nodes containing integer values
            ...
            // Check if tree has been formed as expected
            if (!testTree1.toLevelOrderString().equals("[ 40, 20, 60, 10, 30, 50, 70 ]")) {
                return false;
            }

            // Rotate the tree when parent node is root and the child is right of the parent
            testTree1.rotate(testTree1.root.context[2], testTree1.root);

            // Check if tree has been formed as expected after rotation
            if (!testTree1.toLevelOrderString().equals("[ 60, 40, 70, 20, 50, 10, 30 ]")) {
                System.out.println(testTree1.toLevelOrderString());
                return false;
            }

            if (!testTree1.toInOrderString().equals("[ 10, 20, 30, 40, 50, 60, 70 ]")) {
                System.out.println(testTree1.toInOrderString());
                return false;
            }
    ...
    // This test method will check when parent node is root, and the child is left
    // child of the parent
    public static boolean test2() {
        ...
        try {
            ...
            // Check if tree has been formed as expected
            if (!testTree2.toLevelOrderString().equals("[ 40, 20, 60, 10, 30, 50, 70 ]")) {
                return false;
            }

            testTree2.rotate(testTree2.root.context[1], testTree2.root);

            // Check if tree has been formed as expected after rotation
            if (!testTree2.toLevelOrderString().equals("[ 20, 10, 40, 30, 60, 50, 70 ]")) {
                System.out.println(testTree2.toLevelOrderString());
                return false;
            }

            if (!testTree2.toInOrderString().equals("[ 10, 20, 30, 40, 50, 60, 70 ]")) {
                System.out.println(testTree2.toInOrderString());
                return false;
            }
    ...
    // This test method will check when parent node is not root, and the child is
    // right child of the parent
    public static boolean test3() {
        try {

            // Form a tree with 11 nodes containing integer values
            RedBlackTree<Integer> testTree3 = new RedBlackTree<Integer>();
            ...
            // Check if tree has been formed as expected
            if (!testTree3.toLevelOrderString().equals("[ 30, 27, 33, 25, 28, 31, 35, 24, 26, 34, 36 ]")) {
                System.out.println(testTree3.toLevelOrderString());
                return false;
            }

            testTree3.rotate(testTree3.root.context[2].context[2], testTree3.root.context[2]);

            // Check if tree has been formed as expected after rotation
            if (!testTree3.toLevelOrderString().equals("[ 30, 27, 35, 25, 28, 33, 36, 24, 26, 31, 34 ]")) {
                System.out.println(testTree3.toLevelOrderString());
                return false;
            }

            if (!testTree3.toInOrderString().equals("[ 24, 25, 26, 27, 28, 30, 31, 33, 34, 35, 36 ]")) {
                System.out.println(testTree3.toInOrderString());
                return false;
            }
    ...
    public static boolean test4() {
        try {
            ...
            // Form a tree with 11 nodes containing integer values
            RedBlackTree<Integer> testTree4 = new RedBlackTree<Integer>();
            ...
            // Check if tree has been formed as expected
            if (!testTree4.toLevelOrderString().equals("[ 30, 27, 33, 25, 28, 31, 35, 24, 26, 34, 36 ]")) {
                System.out.println(testTree4.toLevelOrderString());
                return false;
            }

            testTree4.rotate(testTree4.root.context[1].context[1], testTree4.root.context[1]);

            // Check if tree has been formed a expected after rotation
            if (!testTree4.toLevelOrderString().equals("[ 30, 25, 33, 24, 27, 31, 35, 26, 28, 34, 36 ]")) {
                System.out.println(testTree4.toLevelOrderString());
                return false;
            }

            if (!testTree4.toInOrderString().equals("[ 24, 25, 26, 27, 28, 30, 31, 33, 34, 35, 36 ]")) {
                System.out.println(testTree4.toInOrderString());
                return false;
            }
